...
Phát triển nền tảng lõi (Core Platform): Xây dựng Web app xử lý hình ảnh đa góc nhìn (Multi-view), đồng bộ hóa 360 độ từ 6 camera.
Cơ chế Human-in-the-loop & AI Pre-labeling: Tích hợp AI để sinh bản nháp, tối ưu hóa thời gian và công sức của người dùng.
Học chủ động (Active Learning): Phân tích độ tin cậy (Confidence scoring) để ưu tiên con người xử lý các khung hình khó hoặc có độ nhiễu cao.
Kiểm soát chất lượng (QA/QC): Thiết lập quy trình gán nhãn chéo (Cross-check) và Dashboard theo dõi năng suất (TAT).
Chuẩn hóa đầu ra: Đảm bảo dữ liệu xuất bản tuân thủ nghiêm ngặt định dạng metadata của tiêu chuẩn quốc tế nuScenes.
IV. KẾT QUẢ DỰ KIẾN
